...
function updateUnitLabels(units) {
  eggWeightUnit.textContent   = units === 'imperial' ? 'oz (default 2.1)' : 'g (default 60)';
...
function validate() {
  ...
  if (birds > 0 && eggs > birds * 1.3) {
    issues.push('Eggs exceed 130% of flock size. If you collected across ~30 hours, that can happen; otherwise, double-check.');
  } else if (birds > 0 && eggs > birds) {
    issues.push('Note: >100% suggests yesterday’s eggs were included (spillover).');
  }

  const avgEggW = +inputs.avgEggWeight.value || 0;
  if (currentUnits() === 'imperial') {
    if (avgEggW < 1.2 || avgEggW > 3.0) issues.push('Avg egg weight looks unusual for oz (typical ~2.1).');
  } else {
    if (avgEggW < 40 || avgEggW > 80) issues.push('Avg egg weight looks unusual for grams (typical ~60).');
  }
...
function showIssues(list) {
  ...
  issuesEl.innerHTML = '<ul>' + list.map(li => `<li>${li}</li>`).join('') + '</ul>';
}
...
function onCalculate() {
  ...
  sessionNote.textContent = `Saved calculation #${sessionData.calcs.length} for this visit${row.flock ? ` (flock: ${row.flock})` : ''}. Don’t forget to export before leaving.`;
  refreshSessionCounter();
}
...
function exportSessionCsv() {
  ...
  const header = [
    'timestamp','date','flock','units',
    'birdCount','eggCount','avgEggWeight_input','feedConsumed_input',
    'eggWeight_g','feedConsumed_kg','eggMass_kg','fcr','feedPerEgg_g','costPerEgg_usd',
    'bagWeight_input','bagPrice_usd','notes'
  ];
